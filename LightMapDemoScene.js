...
LightMapDemoScene.prototype.Load = function (cb) {
	...
	async.parallel({
		...
		var sphere = window.shapes.generateSphere();
		me.SphereMesh = new Model(
			me.gl, sphere.vertexData, sphere.indexData,
			sphere.normalData, vec4.fromValues(1, 0.62, 0.478,1)
		);

		mat4.translate(
			me.SphereMesh.world, me.SphereMesh.world,
			vec4.fromValues(0.5, 0.5,2)
		);
		...
		me.NoShadowProgram = CreateShaderProgram(
			me.gl,
			getShader(me.gl, 'no-shadow-vertex-shader'),
			getShader(me.gl, 'no-shadow-fragment-shader'),
		);
		...
		me.ShadowProgram = CreateShaderProgram(
			me.gl, 
			getShader(me.gl, 'shadow-vertex-shader'),
			getShader(me.gl, 'shadow-fragment-shader'),
		...
		me.ShadowMapGenProgram = CreateShaderProgram(
			me.gl, 
			getShader(me.gl, 'mapgen-vertex-shader'),
			getShader(me.gl, 'mapgen-fragment-shader'),
